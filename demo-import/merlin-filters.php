...
function metrobank_unset_default_widgets_args( $widget_areas ) {
...
add_filter( 'merlin_unset_default_widgets_args', 'metrobank_unset_default_widgets_args' );


/**
 * Execute custom code after the whole import has finished.
 */
function metrobank_after_import_setup() {
...
add_action( 'merlin_after_all_import', 'metrobank_after_import_setup' );

function metrobank_local_import_files() {
	return array(
		array(
			'import_file_name'         => esc_html__('Main Demo', 'metrobank'),
			...
			'local_import_redux'       => array(
				array(
					...
					'option_name' => 'metrobank_options',
				),
			),
			'local_import_file'        => trailingslashit( get_template_directory() ) . 'demo-import/content/content.xml',
			'import_preview_image_url' => get_template_directory_uri() . '/screenshot.png',
			'import_notice'            => esc_html__( 'After you import this demo, you will have to setup the slider separately.', 'metrobank' ),
			'preview_url'              => esc_url(home_url('/')),
		),
	);
}

add_filter( 'merlin_import_files', 'metrobank_local_import_files' );

function metrobank_child_functions_php( $output, $slug ) {
...
add_filter( 'merlin_generate_child_functions_php', 'metrobank_child_functions_php', 10, 2 );

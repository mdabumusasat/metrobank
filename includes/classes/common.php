...
class Common {
	...
	function blog( $template = 'blog' ) {
		...
		if ( ( $wp_query->is_posts_page && 'blog' == $template ) || $template == 'page' ) {
			...
			$return = [
				'layout'            => metrobank_meta( 'sidebar_sidebar_layout', $page_id, 'right' ),
				'sidebar_type'      => metrobank_meta( 'sidebar_source_type', $page_id, 'd' ),
				'sidebar_elementor' => metrobank_meta( 'sidebar_elementor_template', $page_id ),
				'sidebar'           => metrobank_meta( 'sidebar_page_sidebar', $page_id, 'default-sidebar' ),
				'banner_type'       => metrobank_meta( 'banner_source_type', $page_id ),
				'banner_elementor'  => metrobank_meta( 'banner_elementor_template', $page_id ),
				'banner'            => metrobank_set( metrobank_meta( 'banner_page_background', $page_id ), 'url' ),
				'title'             => metrobank_meta( 'banner_banner_title', $page_id ),
				'enable_banner'     => metrobank_meta( 'banner_page_banner', $page_id ),
			];
		} else {
			...
			$return = [
				...
				'title'         => $options->get( $title ) ? $options->get( $title ) : metrobank_the_title( $template ),
				'banner'        => metrobank_set( $bg, 'url' ),
